<?php
/**
 * Modifications for Roundcube Calendar Plugin:
 *
 * Look for 'find me'
 *
 */
...
//      dbg_error_log( "RRule", " Comparing %s with %s", $this->_text, $lesser->_text );
      // find me: mod: inserted strtotime
      // return ( $this->_text > $lesser->_text );
      return ( strtotime($this->_text) > strtotime($lesser->_text) );
    }
//    dbg_error_log( "RRule", " Comparing %s with %s", $this->_text, $lesser );
    // find me: mod: inserted strtotime
    // return ( $this->_text > $lesser );
    return ( strtotime($this->_text) > strtotime($lesser) );  // These sorts of dates are designed that way...
  }
